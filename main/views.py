...
def siswa_dashboard_view(request):
    ...
    try:
        ...
        messages.error(request, f"Data materi belum diisi oleh Admin. Harap hubungi Guru. (Error: {e})")
        return render(request, 'main/siswa/dashboard_siswa.html', {'next_step': None}) # Kirim context minimal

    # REVISI: Definisikan seluruh alur belajar di satu tempat
    ALUR_BELAJAR = [
        {'nama': 'Modul 1: Karakteristik', 'step': 'modul_1', 'url': redirect('modul', urutan_id=1).url, 'prasyarat': True},
        {'nama': 'Latihan Soal 1', 'step': 'latihan_1', 'url': redirect('latihan_soal', latihan_id=latihan_1_id).url, 'prasyarat': progress.modul_1_completed},
        {'nama': 'Alat: Glosarium', 'step': 'glosarium', 'url': redirect('glosarium').url, 'prasyarat': progress.latihan_1_completed},
        {'nama': 'Modul 2: Klasifikasi', 'step': 'modul_2', 'url': redirect('modul', urutan_id=2).url, 'prasyarat': progress.glosarium_completed},
        {'nama': 'Latihan Soal 2', 'step': 'latihan_2', 'url': redirect('latihan_soal', latihan_id=latihan_2_id).url, 'prasyarat': progress.modul_2_completed},
        {'nama': 'Alat: Lab Mikroskop', 'step': 'mikroskop', 'url': redirect('lab_mikroskop').url, 'prasyarat': progress.latihan_2_completed},
        {'nama': 'Modul 3: 5 Kingdom', 'step': 'modul_3', 'url': redirect('modul', urutan_id=3).url, 'prasyarat': progress.mikroskop_completed},
        {'nama': 'Alat: Database Spesies', 'step': 'database', 'url': redirect('database_spesies').url, 'prasyarat': progress.modul_3_completed},
        {'nama': 'Latihan Soal 3', 'step': 'latihan_3', 'url': redirect('latihan_soal', latihan_id=latihan_3_id).url, 'prasyarat': progress.database_completed},
        {'nama': 'Ujian Akhir Bab', 'step': 'pengayaan', 'url': redirect('pengayaan', pengayaan_id=pengayaan_id).url, 'prasyarat': progress.latihan_3_completed},
    ]
    ...
    skor_terakhir = "-"
    hasil_latihan_terakhir = HasilLatihan.objects.filter(siswa=request.user).order_by('-id').first()
...
def selesai_step_view(request, step_name):
    ...
    valid_steps = [f.name for f in SiswaProgress._meta.get_fields() if isinstance(f, models.BooleanField)]
...
def modul_view(request, urutan_id):
    ...
    if not is_unlocked:
        messages.error(request, 'Selesaikan langkah sebelumnya di alur belajar terlebih dahulu!')
        return redirect('siswa_dashboard')
...
def latihan_soal_view(request, latihan_id):
    ...
    if not is_unlocked:
        messages.error(request, 'Selesaikan modul interaktif sebelumnya terlebih dahulu!')
        return redirect('siswa_dashboard')
...
def glosarium_view(request):
    if request.user.profile.role != 'SISWA': return redirect('guru_dashboard')
    progress = get_object_or_404(SiswaProgress, siswa=request.user)
